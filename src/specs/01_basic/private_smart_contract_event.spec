...
* Execute "contract12"'s `deposit()` function "1" times with arbitrary id and value between original parties
* "Node1" has received transactions from "contract12" which contain "1" log events in state
//* "Node2" has received transactions from "contract12" which contain "10" log events in state
//* Execute "contract23"'s `deposit()` function "10" times with arbitrary id and value between original parties
